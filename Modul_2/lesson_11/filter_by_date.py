...
from_date_input = input("Enter from date:")
to_date_input = input("Enter to date:")

from_date = datetime.strptime(from_date_input, "%Y-%m-%d").date()
to_date = datetime.strptime(to_date_input, "%Y-%m-%d").date()
products_in_range = []
for product in product_data["products"]:
    created_at = datetime.fromisoformat(product["meta"]["createdAt"].replace("Z", "+00:00"))
    created_at_date = created_at.date()

    if from_date <= created_at_date <= to_date:
        products_in_range.append(product)
